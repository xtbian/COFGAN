...
class MetricCalculator():
    ...
    def __init__(self, opt):
        ...
        for metric_type, cfg in self.metric_opt.items():
            ...
            if metric_type.lower() == 'ssim':
                self.ssim_colorspace = cfg['colorspace']
    ...
    def reset_per_sequence(self):
        ...
        self.hr_prev_warp = None
        self.hr_curr_warp = None
    ...
    def compute_sequence_metrics(self, seq_idx, true_seq, pred_seq, warp_prev_seq, warp_curr_seq):
        # clear
        self.reset_per_sequence()
        ...
        for i in range(tot_frm):
            ...
            self.hr_prev_warp = warp_prev_seq[i]
            self.hr_curr_warp = warp_curr_seq[i]
            ...
            warp_h0, warp_w0 = self.hr_prev_warp.shape[:-1]
            warp_h1, warp_w1 = self.hr_curr_warp.shape[:-1]
            min_h, min_w = min(true_h, pred_h, warp_h0, warp_h1), min(true_w, pred_w, warp_w0, warp_w1)
            self.true_img_cur = self.true_img_cur[:min_h, :min_w, :]
            self.pred_img_cur = self.pred_img_cur[:min_h, :min_w, :]
            self.hr_prev_warp = self.hr_prev_warp[:min_h, :min_w, :]
            self.hr_curr_warp = self.hr_curr_warp[:min_h, :min_w, :]
            ...
            self.prev_warp = self.hr_prev_warp
            self.curr_warp = self.hr_curr_warp
    ...
    def compute_frame_metrics(self):
        ...
        for metric_type, opt in self.metric_opt.items():
            ...
            elif metric_type == 'tLP':
                # skip the first frame
                if self.pred_img_pre is not None:
                    tLP100 = self.compute_tLP100()[0, 0, 0, 0].item()
                    metric_dict['tLP'].append(tLP100)
    ...
    def compute_SSIM(self):
        if self.psnr_colorspace == 'rgb':
            ...
            true_img = data_utils.rgb_to_ycbcr(self.true_img_cur)[..., 0]
            pred_img = data_utils.rgb_to_ycbcr(self.pred_img_cur)[..., 0]
        return structural_similarity(true_img, pred_img, data_range=pred_img.max() - pred_img.min())



    def compute_LPIPS(self):   #��GT�ĸ�֪����
        true_img = np.ascontiguousarray(self.true_img_cur)   #תΪ�����洢�����ݣ������ٶȸ���
    ...
    def compute_tLP100(self):    #������֡�ĸ�֪����
        img_cur0 = np.ascontiguousarray(self.true_img_cur)    #תΪ�����洢�����ݣ������ٶȸ���
        img_pre0 = np.ascontiguousarray(self.true_img_pre)
        img_cur1 = np.ascontiguousarray(self.pred_img_cur)
        img_pre1 = np.ascontiguousarray(self.pred_img_pre)

        # to tensor
        cur_img0 = torch.FloatTensor(img_cur0).unsqueeze(0).permute(0, 3, 1, 2)
        pre_img0 = torch.FloatTensor(img_pre0).unsqueeze(0).permute(0, 3, 1, 2)
        cur_img1 = torch.FloatTensor(img_cur1).unsqueeze(0).permute(0, 3, 1, 2)
        pre_img1 = torch.FloatTensor(img_pre1).unsqueeze(0).permute(0, 3, 1, 2)

        # normalize to [-1, 1]
        true_img_cur = cur_img0.to(self.device) * 2.0 / 255.0 - 1.0
        true_img_pre = pre_img0.to(self.device) * 2.0 / 255.0 - 1.0
        pred_img_cur = cur_img1.to(self.device) * 2.0 / 255.0 - 1.0
        pred_img_pre = pre_img1.to(self.device) * 2.0 / 255.0 - 1.0

        with torch.no_grad():
            dist0t = self.dm.forward(true_img_pre, true_img_cur)
            dist1t = self.dm.forward(pred_img_pre, pred_img_cur)

        tLP100 = torch.absolute(dist0t - dist1t) * 100.0
